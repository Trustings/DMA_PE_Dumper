...
#define VMM_MODULE_FLAG_VERSIONINFO      2
    ...
    typedef struct tdVMMOB_TOKEN {
        ...
        union {
                ...
                BYTE pb[12 + (15 * sizeof(DWORD))];  // SECURITY_MAX_SID_SIZE equivalent
        #endif
            } SidUser;
    ...
    typedef struct tdVMM_PROCESS {
     ...
     } LockUpdate, LockPlugin;
    #endif   // Lock used by internal plugins
        DWORD dwPID;
        DWORD dwPPID;
    ...
    */
    EXPORTED_FUNCTION _Success_(return)
        BOOL VMMDLL_Map_GetSystemPfn(_In_ VMM_HANDLE H, _Out_ PMMPFNOB_MAP* ppObPfnMap, _In_ BOOL fExtended, _Out_opt_ PDWORD ppcProgress);
    ...
    //-----------------------------------------------------------------------------
    EXPORTED_FUNCTION  _Success_(return)
        PVMM_PROCESS VMMDLL_ProcessGet(_In_ VMM_HANDLE H, _In_ DWORD dwPID);

    EXPORTED_FUNCTION VOID Ob_XDECREF_NULL(_In_opt_ PVOID* ppOb);
